...
export default function DesktopPostSingle({ post, key }: { post: BlogPost; key: string }) {
  return (
    <div className="col-span-1 mb-5 h-auto transition-all md:hover:scale-105" key={key}>
      <div className="bg-dark flex h-10 w-full items-center justify-center rounded-t-sm">
        <div className="mytext-muted w-1/6 pl-2">
          <Plus size={18} />
        </div>

        <p className="p-xsmall mytext-muted w-4/6 text-center">root@{post.data.createdAt}: ~</p>
      ...
      <div className="flex h-72 flex-col md:h-44 md:flex-row">
        <a
          ...
          className="h-1/2 w-full overflow-hidden md:h-full md:w-1/2"
        >
          <img
